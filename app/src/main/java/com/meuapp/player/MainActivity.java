...
public class MainActivity extends AppCompatActivity {
    ...
    private long lastSeekLogTime = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        timeUpdater = () -> {
            if (vlcPlayer != null && isPlaying && !vlcPreparing) {
                ...
                if (time >= 0 && length > 0) {
                    ...
                    // Log da posição atual a cada 10 segundos
                    long now = System.currentTimeMillis();
                    if (now - lastSeekLogTime > 10000) {
                        lastSeekLogTime = now;
                        long currentByte = time * videoFile.length() / Math.max(length, 1);
                        int currentPiece = pieceLength > 0 ? (int)(currentByte / pieceLength) : 0;
                        debug("📍 Player: " + formatTime(time) + " / " + formatTime(length) + 
                              " | Byte: " + (currentByte/1048576) + "MB | Peça: " + currentPiece + "/" + numPieces);
                    }
                }
            }
        ...
        vlcPlayer.setEventListener(event -> {
            switch (event.type) {
                ...
                case MediaPlayer.Event.Playing: 
                    ...
                    debug("[VLC] ▶ Reproduzindo | Duração: " + (vlcPlayer.getLength()/60000) + "min | Áudios: " + vlcPlayer.getAudioTracks().length + " | Legendas: " + vlcPlayer.getSpuTracks().length);
                    break;
                case MediaPlayer.Event.Paused: 
                    isPlaying = false; 
                    handler.post(() -> btnPlayPause.setText("▶")); 
                    debug("[VLC] ⏸ Pausado em " + formatTime(vlcPlayer.getTime()));
                    break;
                case MediaPlayer.Event.Stopped: 
                    isPlaying = false; vlcPreparing = false; 
                    handler.post(() -> btnPlayPause.setText("▶")); 
                    debug("[VLC] ⏹ Parado");
                    break;
                case MediaPlayer.Event.Buffering: 
                    handler.post(() -> spinnerBar.setVisibility(View.VISIBLE)); 
                    debug("[VLC] 🔃 Buffering... (pos: " + formatTime(vlcPlayer.getTime()) + ")");
                    break;
                ...
                case MediaPlayer.Event.TimeChanged:
                    // TimeChanged é chamado frequentemente - não logar para não poluir
                    break;
                default:
        ...
        btnSkip20.setOnClickListener(v -> {
            if (vlcPlayer != null && !vlcPreparing && videoFile != null && videoFile.exists()) {
                ...
                long length = vlcPlayer.getLength();
                if (length > 0 && targetTime < length) {
                    long targetByte = targetTime * videoFile.length() / length;
                    int targetPiece = pieceLength > 0 ? (int)(targetByte / pieceLength) : 0;
                    debug("⏭ Pulando para 20:00 | Byte alvo: " + (targetByte/1048576) + "MB | Peça alvo: " + targetPiece);
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar s, int p, boolean user) { 
                if (user && vlcPlayer != null && !vlcPreparing && vlcPlayer.getLength() > 0) {
                    long targetTime = (long)(vlcPlayer.getLength() * p / 100.0);
                    long targetByte = targetTime * videoFile.length() / vlcPlayer.getLength();
                    int targetPiece = pieceLength > 0 ? (int)(targetByte / pieceLength) : 0;
                    debug("🖐 SeekBar: " + p + "% | Tempo: " + formatTime(targetTime) + " | Byte: " + (targetByte/1048576) + "MB | Peça: " + targetPiece);
                    seekAbsolute(p);
                }
            }
        ...
        debug("📱 Dispositivo: " + android.os.Build.MODEL);
        debug("🔗 Servidor: http://127.0.0.1:8080/video");
        new Thread(() -> { try { session = new SessionManager(); session.start(); debug("✅ Sessão libtorrent OK"); } catch (Exception e) { debug("❌ Sessão: " + e.getMessage()); } }).start();
        startServer();
    ...
    private void seekRelative(long delta) { 
        ...
        debug("⏪ Seek " + (delta > 0 ? "+" : "") + (delta/1000) + "s → " + formatTime(t));
        vlcPlayer.setTime(t); 
    }
    ...
    private void toggleAudioMenu() {
        ...
        debug("🎵 Listando " + (tracks != null ? tracks.length : 0) + " áudios (atual: " + current + ")");
        if (tracks != null) {
            for (MediaPlayer.TrackDescription t : tracks) {
                if (t.id >= 0) {
                    ...
                    tv.setOnClickListener(v -> { 
                        debug("🎵 Trocando áudio: " + t.name + " (id=" + id + ")");
    ...
    private void toggleSubtitleMenu() {
        ...
        debug("📝 Listando " + (tracks != null ? tracks.length : 0) + " legendas (atual: " + current + ")");
        ...
        if (tracks != null) {
            for (MediaPlayer.TrackDescription t : tracks) {
                if (t.id >= 0) {
                    ...
                    final int id = t.id; 
                    tv.setOnClickListener(v -> { 
                        debug("📝 Trocando legenda: " + t.name + " (id=" + id + ")");
        ...
        serverThread = new Thread(() -> { 
            try { 
                ...
                while (!Thread.interrupted()) { 
                    try { 
                        Socket c = s.accept(); 
                        debug("🔗 Nova conexão HTTP: " + c.getInetAddress() + ":" + c.getPort());
                        new Thread(() -> handleHttp(c)).start(); 
                    } catch (IOException e) { 
                        debug("⚠️ Erro ao aceitar conexão: " + e.getMessage());
                    } 
                } 
                s.close(); 
            } catch (IOException e) { 
    ...
    private void handleHttp(Socket client) {
        String clientId = "HTTP-" + client.getPort();
        debug("📡 [" + clientId + "] Processando requisição...");
        try {
            ...
            if (lines.length == 0 || !lines[0].contains("/video")) { 
                debug("📡 [" + clientId + "] 404 - Não é /video: " + lines[0]);
            ...
            if (videoFile == null || !videoFile.exists()) { 
                debug("📡 [" + clientId + "] 404 - Arquivo não encontrado");
                out.write("HTTP/1.1 404\r\n\r\n".getBytes()); out.flush(); client.close(); return; 
            }
            ...
            if (!hr) {
                long toSend = Math.min(10485760, fs);
                debug("📡 [" + clientId + "] REQUISIÇÃO INICIAL (sem Range) - enviando " + (toSend/1048576) + "MB");
                ...
                synchronized (torrentLock) {
                    if (torrentHandle != null && pieceLength > 0) {
                        ...
                        int ready = 0;
                        for (int i = 0; i <= lastPiece && i < numPieces; i++) if (torrentHandle.havePiece(i)) ready++;
                        debug("📡 [" + clientId + "] Peças 0-" + lastPiece + ": " + ready + "/" + (lastPiece+1) + " prontas");
                        
                        if (ready < lastPiece + 1) {
                            debug("📡 [" + clientId + "] ⏳ Aguardando " + (lastPiece + 1 - ready) + " peças iniciais...");
                            for (int i = 0; i <= lastPiece && i < numPieces; i++) {
                                if (!torrentHandle.havePiece(i)) {
                                    try { torrentHandle.swig().piece_priority_ex(i, (byte)7); torrentHandle.swig().set_piece_deadline(i, 10000); } catch (Exception e) {}
                                }
                            }
                            ...
                            while ((System.currentTimeMillis() - ws) < 15000 && downloading) {
                                ready = 0;
                                for (int i = 0; i <= lastPiece && i < numPieces; i++) if (torrentHandle.havePiece(i)) ready++;
                                if (ready >= lastPiece + 1) break;
                                Thread.sleep(500);
                            }
                            debug("📡 [" + clientId + "] Após espera: " + ready + "/" + (lastPiece+1) + " prontas");
                        }
                    }
                }
                ...
                while (sent < toSend && downloading) {
                    ...
                    if (read <= 0) break;
                    try { out.write(data, 0, read); out.flush(); sent += read; } catch (SocketException e) { 
                        debug("📡 [" + clientId + "] ⚠️ Cliente desconectou após " + (sent/1024) + "KB");
                        break; 
                    }
                }
                raf.close(); out.flush(); client.close();
                if (videoStartTime == 0) videoStartTime = System.currentTimeMillis();
                debug("📡 [" + clientId + "] ✅ Inicial enviado: " + (sent/1024) + "KB");
                return;
            }
            
            if (re == -1 || re >= fs) re = fs - 1;
            long cl = re - rs + 1;
            if (cl > 2097152) { cl = 2097152; re = rs + cl - 1; }
            
            long estMs = (videoDurationMs > 0 && fs > 0) ? rs * videoDurationMs / fs : 0;
            long em = estMs / 60000, es = (estMs / 1000) % 60;
            
            debug("📡 [" + clientId + "] RANGE: bytes " + rs + "-" + (rs+cl-1) + " (" + (cl/1024) + "KB)" +
                  " | Min estimado: " + em + ":" + String.format("%02d", es) +
                  " | Duração detectada: " + (videoDurationMs/60000) + "min");
            
            // 🔥 SEEK FIX: Bloquear até a peça estar disponível
            synchronized (torrentLock) {
                if (torrentHandle != null && pieceLength > 0) {
                    int pn = (int)(rs / pieceLength);
                    
                    boolean hasPiece = torrentHandle.havePiece(pn);
                    debug("📡 [" + clientId + "] Peça " + pn + "/" + numPieces + " → " + (hasPiece ? "JÁ TEMOS ✅" : "NÃO TEMOS ❌"));
                    
                    if (!hasPiece) {
                        debug("📡 [" + clientId + "] ⏳ AGUARDANDO peça " + pn + " (min " + em + ":" + String.format("%02d", es) + ")");
                        
                        // Verificar disponibilidade na rede
                        int_vector avail = new int_vector();
                        try { torrentHandle.swig().piece_availability(avail); } catch (Exception e) {}
                        int peersWithPiece = (pn < avail.size()) ? avail.get(pn) : -1;
                        debug("📡 [" + clientId + "] Peers com peça " + pn + ": " + peersWithPiece);
                        ...
                        try {
                            ...
                            debug("📡 [" + clientId + "] ⚠️ Erro ao definir prioridade: " + e.getMessage());
                        }
                        ...
                        while (!torrentHandle.havePiece(pn) && (System.currentTimeMillis() - ws) < 30000 && downloading) {
                            ...
                            if (waitLogs % 10 == 0) {
                                ...
                                debug("📡 [" + clientId + "] ⏳ " + (waitLogs/2) + "s aguardando peça " + pn + "...");
                            }
                        }
                        
                        if (torrentHandle.havePiece(pn)) {
                            debug("📡 [" + clientId + "] ✅ Peça " + pn + " CHEGOU em " + ((System.currentTimeMillis()-ws)/1000) + "s!");
                        } else {
                            debug("📡 [" + clientId + "] ⏰ TIMEOUT peça " + pn + " após " + ((System.currentTimeMillis()-ws)/1000) + "s");
                        }
                    }
                }
            }
            ...
            while (sent < cl && downloading) {
                ...
                try { out.write(buf, 0, read); out.flush(); sent += read; } catch (SocketException e) { 
                    debug("📡 [" + clientId + "] ⚠️ Cliente desconectou durante envio");
                    break; 
                }
            }
            bytesServed += sent;
            ...
            out.flush();
            client.close();
            debug("📡 [" + clientId + "] ✅ Range enviado: " + (sent/1024) + "KB | Total servido: " + (bytesServed/1048576) + "MB");
            
        } catch (Exception e) { 
            try { client.close(); } catch (IOException ex) {} 
            debug("📡 [" + clientId + "] ❌ Erro: " + e.getMessage());
        }
    }
    ...
    private void startDownload(String source) {
        ...
        new Thread(() -> {
            try {
                ...
                debug("🔗 Adicionando torrent à sessão...");
                ...
                debug("🔗 Torrent adicionado, aguardando metadados...");
                ...
                if (torrentHandle != null && torrentHandle.isValid() && torrentHandle.torrentFile() != null) {
                    ...
                    debug("📊 Torrent: " + (totalSize/1048576) + "MB | Peças: " + np + " x " + (pl/1024) + "KB | Peers: " + st.getNum_peers() + " | Seeds: " + st.getNum_seeds());
                    debug("📊 Download sequencial: SIM | Limite: 3MB/s");
                    ...
                    while (wt < 120 && downloading) { Thread.sleep(500); complete = 0; wt++;
                        ...
                        if (!shown && complete >= meta) { shown = true; debug("✅ Cabeçalho OK! " + complete + "/" + meta + " em " + (wt/2) + "s");
                            byte_vector np2 = new byte_vector(); for (int i = 0; i < np; i++) np2.add((byte)1);
                            torrentHandle.swig().prioritize_pieces_ex(np2);
                            debug("📋 Prioridades restauradas para download normal");
                            ...
                            for (int i = 0; i < 30; i++) { File f = find(new File(savePath)); if (f != null && f.length() > 1048576) { byte[] hdr = new byte[8]; try { new RandomAccessFile(f, "r").read(hdr); } catch (Exception e2) { continue; }
                                if ((hdr[4]=='f'&&hdr[5]=='t'&&hdr[6]=='y'&&hdr[7]=='p') || ((hdr[0]&0xFF)==0x1A&&hdr[1]==0x45&&hdr[2]==(byte)0xDF&&hdr[3]==(byte)0xA3)) { videoFile = f;
                                    handler.post(() -> { btnWatch.setText("🎬 ASSISTIR"); btnWatch.setVisibility(View.VISIBLE); bufferBar.setVisibility(View.GONE); });
                                    debug("📁 Arquivo: " + f.getName() + " (" + (f.length()/1048576) + "MB)");
                                    debug("📁 Formato: " + ((hdr[4]=='f') ? "MP4/MOV" : "MKV/WebM"));
                                    break;
                                }
