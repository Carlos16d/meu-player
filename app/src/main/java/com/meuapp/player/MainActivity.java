...
public class MainActivity extends AppCompatActivity {
    ...
    private volatile boolean handleValid = false;
    private volatile boolean downloading;
    private volatile File videoFile;
    private long videoFileOffset = 0;
    private Handler handler;
    private Thread serverThread, prefetchThread;
    ...
    private boolean havePieceSafe(int piece) {
        if (!handleValid || torrentHandle == null) return false;
        synchronized (torrentLock) {
            try { return torrentHandle.havePiece(piece); } catch (Exception e) {}
            return false;
        }
    }
    
    private void safeSwig(Runnable action) {
        if (!handleValid || torrentHandle == null) return;
        synchronized (torrentLock) {
    ...
    private void playWithExoPlayer(String url) {
        try {
            ...
            handler.post(() -> { playerView.setVisibility(View.VISIBLE); playerControls.setVisibility(View.VISIBLE); centerControls.setVisibility(View.VISIBLE); btnSkip20.setVisibility(View.VISIBLE); bufferProgress.setVisibility(View.VISIBLE); handler.post(timeUpdater); });
    ...
    private void handleHttp(Socket client) {
        try {
            ...
            if (!hr) {
                if (fs < 1048576) { out.write("HTTP/1.1 503 Retry-After: 5\r\n\r\n".getBytes()); out.flush(); client.close(); return; }
                long toSend = Math.min(3145728, fs);
                String resp = "HTTP/1.1 200 OK\r\nContent-Type: video/x-matroska\r\nAccept-Ranges: bytes\r\nContent-Length: " + fs + "\r\nAccess-Control-Allow-Origin: *\r\n\r\n";
                out.write(resp.getBytes()); out.flush();
                RandomAccessFile raf = new RandomAccessFile(videoFile, "r");
                byte[] data = new byte[262144]; long sent = 0;
                while (sent < toSend && downloading) { int tr = (int)Math.min(data.length, toSend - sent); int read = raf.read(data, 0, tr); if (read <= 0) break; try { out.write(data, 0, read); out.flush(); sent += read; } catch (SocketException e) { break; } }
    ...
    private void startDownload(String source) {
        downloading = true; videoFile = null; torrentHandle = null; handleValid = false; videoFileOffset = 0; videoStartTime = 0; pieceLength = 0; numPieces = 0; totalSize = 0; videoDurationMs = 0;
        ...
        new Thread(() -> {
            try {
                ...
                synchronized (torrentLock) { torrent_handle_vector h = session.swig().get_torrents(); if (h.size() > 0) { torrentHandle = new TorrentHandle(h.get(0)); handleValid = true; } }
                int w = 0; while (w < 60 && downloading) { Thread.sleep(1000); w++; if (handleValid && torrentHandle != null && torrentHandle.torrentFile() != null) break; }
                
                if (handleValid && torrentHandle != null && torrentHandle.torrentFile() != null) {
                    ...
                    int totalMeta = headerPieces + cuePieces;
                    debug("📊 " + (totalSize/1048576) + "MB " + np + "p H:0-" + (headerPieces-1) + " C:" + cueStart + "-" + (np-1) + " (" + totalMeta + " peças)");
                    
                    // Fase 1: Baixar metadados (cabeçalho + cues)
                    final int hdr = headerPieces, cueS = cueStart;
                    safeSwig(() -> {
                        byte_vector hp = new byte_vector();
                        for (int i = 0; i < np; i++) hp.add((byte)((i < hdr || i >= cueS) ? 7 : 0));
                        torrentHandle.swig().prioritize_pieces_ex(hp);
                        for (int i = 0; i < hdr; i++) torrentHandle.swig().set_piece_deadline(i, 500);
                        for (int i = cueS; i < np; i++) torrentHandle.swig().set_piece_deadline(i, 500);
                    });
                    
                    int complete = 0, wt = 0; boolean metaReady = false;
                    while (wt < 300 && downloading && !metaReady) { Thread.sleep(500); complete = 0; wt++;
                        for (int i = 0; i < headerPieces; i++) if (havePieceSafe(i)) complete++;
                        for (int i = cueStart; i < np; i++) if (havePieceSafe(i)) complete++;
                        if (complete >= totalMeta) { metaReady = true; debug("✅ Metadados: " + complete + "/" + totalMeta + " " + (wt/2) + "s"); }
                    }
                    
                    if (metaReady) {
                        // Fase 2: Baixar +30 segundos de vídeo antes de liberar
                        int pecas30s = (int)((30.0 / 1420) * np); // ~30s para vídeo de 23:40
                        if (pecas30s < 5) pecas30s = 5;
                        if (pecas30s > 30) pecas30s = 30;
                        int totalInicio = headerPieces + pecas30s;
                        debug("🎬 Baixando " + pecas30s + " peças iniciais (~30s de vídeo)...");
                        
                        final int hdr2 = headerPieces, cueS2 = cueStart, p30 = pecas30s;
                        safeSwig(() -> {
                            byte_vector np2 = new byte_vector();
                            for (int i = 0; i < np; i++) {
                                if (i < hdr2 + p30 || i >= cueS2) np2.add((byte)7);
                                else np2.add((byte)0);
                            }
                            torrentHandle.swig().prioritize_pieces_ex(np2);
                            for (int i = hdr2; i < hdr2 + p30; i++) torrentHandle.swig().set_piece_deadline(i, 1000);
                        });
                        
                        int ready = 0, wt2 = 0; boolean inicioReady = false;
                        while (wt2 < 120 && downloading && !inicioReady) { Thread.sleep(500); ready = 0; wt2++;
                            for (int i = 0; i < totalInicio; i++) if (havePieceSafe(i)) ready++;
                            if (wt2 % 4 == 0) debug("   🎬 " + ready + "/" + totalInicio + " (" + (wt2/2) + "s)");
                            if (ready >= totalInicio) { inicioReady = true; debug("✅ Início pronto: " + ready + "/" + totalInicio + " " + (wt2/2) + "s"); }
                        }
                        
                        // Fase 3: Restaurar prioridades e liberar player
                        final int hdr3 = headerPieces, cueS3 = cueStart, p302 = pecas30s;
                        safeSwig(() -> {
                            byte_vector np3 = new byte_vector();
                            for (int i = 0; i < np; i++) {
                                if (i < hdr3 + p302 || i >= cueS3) np3.add((byte)4);
                                else np3.add((byte)0);
                            }
                            torrentHandle.swig().prioritize_pieces_ex(np3);
                        });
                        ...
                        for (int i = 0; i < 60; i++) { 
                            ...
                            if (f != null && f.length() > 5242880) {
                                byte[] hdr3 = new byte[8]; 
                                try { new RandomAccessFile(f, "r").read(hdr3); } catch (Exception e2) { continue; }
                                if ((hdr3[4]=='f'&&hdr3[5]=='t'&&hdr3[6]=='y'&&hdr3[7]=='p') || ((hdr3[0]&0xFF)==0x1A&&hdr3[1]==0x45&&hdr3[2]==(byte)0xDF&&hdr3[3]==(byte)0xA3)) { 
                                    videoFile = f;
        ...
        // ==================== PREFETCH VIA HANDLER (Main Thread) ====================
        prefetchThread = new Thread(() -> {
            while (downloading) {
                try { Thread.sleep(2000); } catch (Exception e) {}
                if (!handleValid || videoFile == null || pieceLength <= 0) continue;
                
                // Postar na main thread para acessar ExoPlayer
                handler.post(() -> {
                    if (exoPlayer == null || !exoPlayer.isPlaying() || exoPlayer.getDuration() <= 0) return;
                    ...
                    int pecasPara10s = pecasPara30s / 3;
                    
                    final int cp = currentPiece, p10 = pecasPara10s, p30 = pecasPara30s;
                    safeSwig(() -> {
                        for (int i = cp + 1; i <= cp + p10 && i < numPieces; i++)
                            if (!torrentHandle.havePiece(i)) { torrentHandle.swig().piece_priority_ex(i, (byte)7); torrentHandle.swig().set_piece_deadline(i, 15000); }
                        for (int i = cp + p10 + 1; i <= cp + p30 && i < numPieces; i++)
    ...
    private void stop() { 
        downloading = false; handleValid = false;
    ...
    @Override protected void onDestroy() { stop(); if (serverThread != null) serverThread.interrupt(); if (prefetchThread != null) prefetchThread.interrupt(); if (session != null) session.stop(); if (exoPlayer != null) exoPlayer.release(); super.onDestroy(); }
}