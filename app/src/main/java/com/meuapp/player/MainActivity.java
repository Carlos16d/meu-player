...
public class MainActivity extends AppCompatActivity {
    ...
    private Button btnPlayPause, btnSeekBack, btnSeekForward;
    ...
    private void handleHttp(Socket client) {
        try {
            ...
            if (!hr) {
                if (fs < 1048576) { out.write("HTTP/1.1 503\r\n\r\n".getBytes()); out.flush(); client.close(); return; }
                long toSend = Math.min(5242880, fs);
    ...
    private void startDownload(String source) {
        downloading = true; videoFile = null; torrentHandle = null; pieceLength = 0; numPieces = 0; totalSize = 0; lastDownloadLog = 0; videoDurationMs = 0;
        ...
        // Monitor de download SIMPLES (apenas log, sem prefetch)
        new Thread(() -> { 
            while (downloading) { 
