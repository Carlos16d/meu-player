...
public class MainActivity extends AppCompatActivity {
    ...
    private void handleHttp(Socket client) {
        try {
            ...
            // 🔥 SEEK FIX: Aguardar a peça necessária antes de responder
            synchronized (torrentLock) {
                if (torrentHandle != null && pieceLength > 0) {
                    ...
                    if (!torrentHandle.havePiece(pieceNeeded)) {
                        ...
                        debug("⏳ SEEK min " + em + ":" + String.format("%02d", es) + " (peça " + pieceNeeded + ") - AGUARDANDO...");
                        
                        // Prioridade MÁXIMA para esta peça e vizinhas
                        try {
                            torrentHandle.swig().piece_priority_ex(pieceNeeded, (byte)7);
                            torrentHandle.swig().set_piece_deadline(pieceNeeded, 30000);
                            for (int i = pieceNeeded - 2; i <= pieceNeeded + 8; i++) {
                        ...
                        while (!torrentHandle.havePiece(pieceNeeded) && (System.currentTimeMillis() - waitStart) < 30000 && downloading) {
                            Thread.sleep(500);
                            try { torrentHandle.swig().set_piece_deadline(pieceNeeded, 30000); } catch (Exception e) { break; }
                        }
                        
                        if (torrentHandle.havePiece(pieceNeeded)) {
                            debug("✅ Peça " + pieceNeeded + " chegou em " + ((System.currentTimeMillis() - waitStart)/1000) + "s");
                        } else {
                            debug("⏰ Timeout peça " + pieceNeeded + " - enviando parcial");
                        }
                    }
                }
            }
