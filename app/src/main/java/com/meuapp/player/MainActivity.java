...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar s, int p, boolean user) { 
                if (user && exoPlayer != null && exoPlayer.getDuration() > 0) {
                    exoPlayer.seekTo((long)(exoPlayer.getDuration() * p / 100.0)); 
                }
            }
    ...
    private void handleHttp(Socket client) {
        try {
            ...
            synchronized (torrentLock) {
                if (torrentHandle != null && pieceLength > 0) {
                    ...
                    if (!torrentHandle.havePiece(pn)) {
                        ...
                        if (torrentHandle.havePiece(pn)) debug("✅ p" + pn + " " + ((System.currentTimeMillis()-ws)/1000) + "s");
                    }
                }
            }
            ...
            RandomAccessFile raf = new RandomAccessFile(videoFile, "r"); raf.seek(rs);
            byte[] buf = new byte[65536]; long sent = 0;
            while (sent < cl && downloading) {
                ...
                out.write(buf, 0, read); out.flush(); sent += read;
            }
            raf.close(); out.flush(); client.close();
        ...
        new Thread(() -> {
            try {
                ...
                if (torrentHandle != null && torrentHandle.isValid() && torrentHandle.torrentFile() != null) {
                    ...
                    debug("📊 " + (totalSize/1048576) + "MB " + np + "p Peers:" + torrentHandle.swig().status().getNum_peers());
                    
                    int header = Math.min(20, np), cue = Math.min(15, np), cueStart = Math.max(0, np - cue);
                    int total = header + cue;
                    debug("📋 H:0-" + (header-1) + " C:" + cueStart + "-" + (np-1) + " =" + total);
                    
                    byte_vector hp = new byte_vector();
                    for (int i = 0; i < np; i++) hp.add((byte)((i < header || i >= cueStart) ? 7 : 0));
                    torrentHandle.swig().prioritize_pieces_ex(hp);
                    for (int i = 0; i < header; i++) torrentHandle.swig().set_piece_deadline(i, 500);
                    ...
                    while (wt < 120 && downloading) { Thread.sleep(500); complete = 0; wt++;
                        for (int i = 0; i < header; i++) if (torrentHandle.havePiece(i)) complete++;
                        for (int i = cueStart; i < np; i++) if (torrentHandle.havePiece(i)) complete++;
                        if (wt % 4 == 0) debug("   📋 " + complete + "/" + total + " " + (wt/2) + "s");
                        if (!shown && complete >= total) { shown = true; debug("✅ OK " + complete + "/" + total);
                            ...
                            for (int i = 0; i < 30; i++) { File f = find(new File(savePath)); if (f != null && f.length() > 1048576) { byte[] hdr = new byte[8]; try { new RandomAccessFile(f, "r").read(hdr); } catch (Exception e2) { continue; }
                                if ((hdr[4]=='f'&&hdr[5]=='t'&&hdr[6]=='y'&&hdr[7]=='p') || ((hdr[0]&0xFF)==0x1A&&hdr[1]==0x45&&hdr[2]==(byte)0xDF&&hdr[3]==(byte)0xA3)) { videoFile = f;
                                    ...
                                    debug("📁 " + f.getName() + " " + (f.length()/1048576) + "MB"); break;
                                }
